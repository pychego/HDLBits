...
// 这个是我仿真中的数据, 不是ZJG的补偿矩阵
// 注意, 此雅可比句矩阵后三列数据较大, 是弧度雅可比矩阵, 但是输入是角度, 没问题!! 这样会让角度误差减小
// 2024.8.26 我又设置的保守了一些,还是使用角度雅可比矩阵(后三列数据变小)
void CompensationJacobian(float x, float y, float z, float a, float b, float c, float J[6][6])
{
    ...
    J[0][1] = x67 / x54;
    J[0][2] = -x66 / x54;
    J[0][3] = -(x66 * (x95 - x92 + x91 + x90 + x89) - x67 * (x100 - x88 + x87 + x86 + x85)) / x54_180;
    J[0][4] = (x66 * (x24 - x2 + x23) - x68 * (4 * cosb + x49 + x28) + x67 * (x19 - x1 + x17)) / x54_180;
    J[0][5] = (x67 * (x34 + x33 + x16 - x9) - x66 * (x50 + x30 - x22 + x14) + x68 * (x36 - x29)) / x54_180;

    J[1][0] = x65 / x53;
    J[1][1] = x64 / x53;
    J[1][2] = -x63 / x53;
    J[1][3] = -(x63 * (x95 - x81 + x82 + x79 + x80) - x64 * (x100 - x78 + x77 + x75 + x76)) / x53_180;
    J[1][4] = (x64 * (x12 - x1 + x21) - x65 * (4 * cosb + x31 + x41) + x63 * (x15 - x2 + x26)) / x53_180;
    J[1][5] = (x64 * (x48 + x35 + x20 - x3) - x63 * (x32 + x43 - x25 + x7) + x65 * (x37 - x42)) / x53_180;

    J[2][0] = -x74 / x56;
    J[2][1] = x72 / x56;
    J[2][2] = x73 / x56;
    J[2][3] = (x73 * (x99 + x95 + x98 + x97 - x96) - x72 * (x103 - x100 + x104 - x102 + x101)) / x56_180;
    J[2][4] = (x73 * (x2 - x18 + x11) - x72 * (x1 - x13 + x6) + x74 * (4 * cosb - x38 + x44)) / x56_180;
    J[2][5] = (x73 * (x46 - x39 + x10 + x8) + x74 * (x47 + x45) - x72 * (x40 - x27 + x5 + x4)) / x56_180;

    J[3][0] = -x62 / x52;
    J[3][1] = -x60 / x52;
    J[3][2] = x61 / x52;
    J[3][3] = -(x61 * (x98 - x95 - x99 + x97 + x96) - x60 * (x104 - x100 - x103 + x102 + x101)) / x52_180;
    J[3][4] = (x60 * (x1 + x13 + x6) + x61 * (x2 + x18 + x11) - x62 * (x38 - 4 * cosb + x44)) / x52_180;
    J[3][5] = (x62 * (x47 - x45) - x61 * (x39 + x46 - x10 + x8) + x60 * (x27 + x40 + x5 - x4)) / x52_180;

    J[4][0] = x59 / x51;
    J[4][1] = -x57 / x51;
    J[4][2] = -x58 / x51;
    J[4][3] = -(x57 * (x78 + x100 + x77 - x75 + x76) - x58 * (x81 - x95 + x82 + x79 - x80)) / x51_180;
    J[4][4] = -(x59 * (4 * cosb + x31 - x41) - x57 * (x1 + x12 - x21) + x58 * (x2 + x15 - x26)) / x51_180;
    J[4][5] = (x58 * (x43 - x32 + x25 + x7) + x59 * (x37 + x42) - x57 * (x35 - x48 + x20 + x3)) / x51_180;

    J[5][0] = x71 / x55;
    J[5][1] = x70 / x55;
    J[5][2] = -x69 / x55;
    J[5][3] = (x69 * (x92 - x95 + x91 + x90 - x89) + x70 * (x88 + x100 + x87 - x86 + x85)) / x55_180;
    J[5][4] = -(x71 * (4 * cosb + x49 - x28) + x69 * (x2 + x24 - x23) + x70 * (x1 + x19 - x17)) / x55_180;
    J[5][5] = (x69 * (x30 - x50 + x22 + x14) + x70 * (x33 - x34 + x16 + x9) + x71 * (x36 + x29)) / x55_180;
}
